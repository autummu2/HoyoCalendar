...
def is_reprint(topic: str, sub: str) -> bool:
    """这条帖子是不是「复刻 / 返场」帖（标题要收成「活动名 + 限时复刻」，RULES §4.2）。

    只看**栏目**与子标题里的 `限时返场`，不查正文：窗口内 7 篇复刻 / 返场帖的信号都在这
    两处（栏目 `往期复刻` / `轮替女神之影`，或子标题末尾的 `限时返场`），而正文里提「复刻」
    的只有其中 5 篇（5 篇 `往期复刻`，2 篇轮替返场帖一次都没提）—— 多这一层来源不多认出
    一条，却给标题（去重主键 `keys.event_key` 的一部分）多添一处会变的判据。
    """
    return topic.startswith(rules.REPRINT_TOPIC_PREFIXES) or "限时返场" in sub
...
def title_of(subject: str, text: str, etype: str) -> str:
    ...
    （`往期复刻` / `轮替女神之影` / `商城上新` 都会撞名），拿它当标题会撞。
      ...
      - 复刻 / 返场帖（`往期复刻` / `轮替女神之影`）：收掉子标题末尾的官方套话
        （`活动限时复刻` / `活动女神之影限时复刻` / `限时返场`），再在活动名后面、
        **引号外面**接 `限时复刻` —— `「濯影拾辉」活动限时复刻` → `「濯影拾辉」限时复刻`。
        首开那一期的标题没有这个后缀，两期因此分得开
      - 卡池：`角色+稀有度+【思绪名】`（`夏彦SSR【换日线】`），子标题里找不到就退回子标题
        （复刻卡池由上一支接管，子标题本身就是活动名。
    ...
    """
    topic, sub = split_subject(subject)
    if is_reprint(topic, sub):
        return f"{rules.REPRINT_TAIL_RE.sub('', sub) or topic}{rules.REPRINT_SUFFIX}"
    if etype == "卡池":
...
def detect_tags(subject: str, text: str) -> list[str]:
    ...
    **标题命中就不看正文**：标题是「这篇指向谁」（`夏彦SSR【夏花永隽】限时复刻`），正文是上下文
    —— 轮替返场帖的正文会把整个轮替池的角色都列一遍，只看正文会把两条轮替帖打成四人全中。
    标题一个标签都认不出时才退到正文：复刻帖的标题只有活动名（`「濯影拾辉」限时复刻`
    ...
    `「NXX-冰原上的审判」限时复刻` 的 SSR 与四个人名都只有正文有，手工也正是这么记的。
    """

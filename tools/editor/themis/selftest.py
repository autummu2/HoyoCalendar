...
TITLES = [
    ...
    # 复刻 / 返场：统一收成「活动名 + 限时复刻」，后缀接在引号外面，官方套话先收掉（裁定 32）
    ("往期复刻丨「濯影拾辉」活动限时复刻", "常规活动", "「濯影拾辉」限时复刻"),
    ("往期复刻丨「心有所欲」活动限时复刻", "常规活动", "「心有所欲」限时复刻"),
    ("往期复刻丨「NXX-冰原上的审判」活动女神之影限时复刻", "卡池",
     "「NXX-冰原上的审判」限时复刻"),
    # 轮替返场也走复刻分支，所以不会取成「轮替女神之影更新」那个栏目名
    ("轮替女神之影更新丨夏彦SSR【夏花永隽】限时返场", "卡池", "夏彦SSR【夏花永隽】限时复刻"),
    # 子标题里既无「…」也无套话：收不出活动名，直接接后缀
    ("往期复刻丨莫弈生日系列限时复刻", "卡池", "莫弈生日系列限时复刻"),
    # 靠主题兜底的：子标题里没有「…」
...
def test_is_reprint():
    # 两个复刻栏目 + 子标题里的 `限时返场`
    check("往期复刻栏目", parse.is_reprint("往期复刻", "「心有所欲」活动限时复刻"), True)
    check("轮替栏目", parse.is_reprint("轮替女神之影", "莫弈SSR【莫负良辰】限时返场"), True)
    # ⚠️ `限时返场` 这一支目前没有单独的样本：窗口内两篇带它的帖子，栏目也已经命中。
    # 留着是因为它才是「返场」的字面信号（栏目改叫别的名字时还得靠它）。
    # 首开帖一律不算：栏目就是活动名，子标题里没有复刻 / 返场字样
    for topic, sub in (("青葱寄愿", "限时活动开启 签到领取甜心家装图纸及【女神之泪】"),
                       ("岁悦同欢·莫弈篇", "免费获取多重生日限定福利"),
                       ("星航寻梦", "限时活动预告 左然MR【幻航】开放获取"),
                       ("换日线", "活动女神之影开启 夏彦SSR【换日线】概率提升")):
        check(f"首开不算复刻 {topic[:8]}", parse.is_reprint(topic, sub), False)
...
def test_pre_screen():
    ...
    def recording_body_of(post_id):
        fetched.append(str(post_id))
        return body_of(post_id)
    ...
    # 代价说清楚：那篇帖子若**唯一**对应这条活动，它的候选就不再产出 —— 与
    # apply_events 的「已存在就跳过」等价（条目已经在数据文件里了）
    for pid in ("77934380", "77910710"):
        check(f"已录入帖唯一对应的候选不再产出 {pid}",
              pipeline.collect([p for p in NEWS if str(p["post_id"]) == pid], [],
                               body_of, existing), [])
    check("同活动另一篇帖子照旧产出",
          [e["title"] for e in pipeline.collect(
              [p for p in NEWS if str(p["post_id"]) == "77933880"], [], body_of, existing)],
          ["「爱的未定式·夏彦篇」"])
...
def main():
    for fn in (test_title_skip, test_period, test_classify, test_titles, test_is_reprint,
               test_tags, test_version_notes, test_collect, test_pre_screen,
               test_identity, test_matches_existing):
        fn()

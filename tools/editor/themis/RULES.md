...
> `selftest.py` 离线自检 106 项），实测抓取见 [PLAN.md](PLAN.md) §6。
> 本文的每条判据都有对应用例断言，改判据要连 `selftest.py` 一起改。
>
> 所有判据都来自 2026-09-25 的实测抓取（米游社 95 篇正文 + B站 90 / **500** 条动态）。
> 标 **🔒 已定** 的是已裁定的判据 —— 裁定清单（31 行）在 [PLAN.md](PLAN.md) §5，
...
flowchart TD
    ...
    B1["米游社资讯栏 type=3<br/>50 条 = 5 周（硬上限）"] --> B2{"① 标题命中负向名单?<br/>§2.2"}
...
### 2.2 分类决策树（两道预筛闸 + 两次归类）

**两道闸的顺序很重要：先看标题，后取正文。** ① 只看标题就能判掉 19/50 篇，
省掉 19 次正文请求（未定正文接口虽然目前不风控，但没理由白花）。

```mermaid
flowchart TD
    S["资讯栏一条帖子<br/>subject = 主题丨子标题"] --> G1{"① 标题命中负向名单?<br/>第一道闸"}
...
```mermaid
flowchart TD
    A["资讯栏帖子 (type=3)"] --> B{"① 标题命中负向名单?<br/>累充/商城/联名/周边/积分/PV丨"}
...
🔒 **卡池的身份带开始日期** `(类型, 标题, 开始日期)`（2026-09-25 裁定，见 §4.2）：
未定的卡池会**返场**且标题逐字相同，三端那种不带日期的主键会把返场静默跳过。
...
- 🔒 **标题取「活动名」，不取整条 `主题丨子标题`**（2026-09-25 裁定）。
  ...
  取法在 `parse.title_of`：复刻帖取子标题原样（子标题本身以活动名开头）、
...
- 身份（`common.keys.event_key`）：
  ...
  - 🔒 **卡池 → `(类型, 标题, 开始日期)`，带上日期**（2026-09-25 裁定）。
    ...
    **不含日期**，会把返场静默跳过。手工数据里 `「NXX-冰原上的审判」活动女神之影
    限时复刻`（卡池，8/27）就是这个风险已经踩到边界的例子。
...
预筛已经在 §2.2 讲完（两道闸），本节只说 `pending`（`common.keys.PENDING_FIELD`）。
...
- **来源顺序：标题命中就不看正文**。标题是「这篇指向谁」（`夏彦SSR【夏花永隽】`），
  正文是上下文 —— 轮替返场帖的正文会把整个轮替池的角色列一遍，只看正文会把两条
  轮替帖打成四人全中（实测正是如此）。标题一个标签都认不出时才退到正文：
  复刻帖标题只有活动名（`「濯影拾辉」活动限时复刻` 的左然 MR 只在正文里）、
  七夕/多人活动也只有正文才有名字。
...
| 卡池 | 标题取 `角色+稀有度+【思绪名】`；身份**带开始日期**（会返场） | 三端卡池主键不含日期；未定返场时标题逐字相同（§4.2） |

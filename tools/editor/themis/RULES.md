...
> `selftest.py` 离线自检 124 项），实测抓取见 [PLAN.md](PLAN.md) §6。
...
不取正文。它省的是正文请求，**也**可能让候选少几条：窗口内命中 2 篇（31 → 29 次请求、
18 → 17 条候选），少的那条正是已录入的 —— 落盘结果不变（`apply_events` 本来也会跳过它）。
命中数随管线自己落的条目增多而变多。
...
**省下来的是正文请求**：窗口内命中 2 篇（31 → 29），因为手工那 19 条里只有 4 条带帖子
id；管线自己落的条目**都带 `source_url`**，命中率随轮次涨上来 —— 正文请求正是这条线最该
省的地方（§4.6）。候选随之 18 → 17（少的是已录入那条，见上段）。

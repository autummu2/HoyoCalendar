...
> **原神是三条线里唯一没有「总纲」的**：米游社没有一份「每版本开服当天发、含本版全部活动
> 与下版本日期」的公告。所以它需要一套**周期性推理**（`genshin/inference.py`）补版本更新、
> 前瞻直播（B站）、深境螺旋、幻想真境剧诗的日期，卡池日期也是先推后校。这是原神与另两条
> 线最大的结构差异——不是实现风格不同，是**信息源少了一整块**。
>
> 下面每条规则都由真实公告核对过；与另两条线不同，原神**没有离线自检**（见 [PLAN.md](PLAN.md) §2.1）。
...
`python maintain.py`（跑三条管线），或单跑 `python genshin/pipeline.py`：
...
> ⚠️ **原神这一段的顺序与另两条线不同**：订正 / 校准排在**日期闸之前**，取色排在**最后**。
...
`装扮上架` 是 2026-09-24 补的：千星奇域的装扮颂愿池，时段挂在 `〓活动颂愿-N介绍〓`
（不是 `〓活动时间〓`），落不了日期。两篇姊妹篇只差一个字——`「奇域秘藏·暄霁霄练」奇偶装扮`
**上新**（早在名单里）与 `「奇域秘藏·幻夜游烛」奇偶装扮`**上架**（漏网，每轮白抓一次正文）。

**高难关键词**另有一组（`CHALLENGE_KEYWORDS`）：`危战`、`深境螺旋`、`深渊`、
`幻想真境剧诗`、`混沌回忆`、`忘却之庭` —— 命中即不当作常规活动（高难有独立类型）。

> ⚠️ **不可用宽泛的 `祈愿` 二字排除活动**：会误杀标题里恰好带「祈愿」的常规活动。实例：
> `「巡历雪境，凝铸锋锐」：自选邀请「奔行世间」常驻祈愿5星角色` 曾被整条丢掉。所以卡池
> 用**精确特征**排除（`概率` / `溯光祈愿` / `集录祈愿`）。
...
- B站动态 `〓活动时间〓` 两种格式，**首日期 = start、末日期 = end**：
  ...
  - 两条例外（2026-09-24 修正，此前两条都记错）：
    - **`end` 只有时刻是 `03:59` 才减一天**，其余照抄（`14:59` / `17:59` / `5:59` 的结束日就是
      当天）。旧实现不看时刻、一律减一天，把 `~ 2026/11/03 14:59` 记成了 `11-02`。
    - **段里只有一个日期（起点写作版本代称）时 start 留空**，交给下面的米游社兜底算版本更新日：
      `7.1版本更新后~2026/11/04 5:59`。旧实现拿那个唯一日期同时当起止，落成 `11-04 ~ 11-03`。
...
实例 `「虹旅藏金·耀星烁熠」`。取哪一段作为整条活动的起止没有明确答案（首段结束是「版本结束」、
次段是显式日期）。**已确认处置：不加解析分支，不记录，由人工在编辑器里补录**。
...
（旧版「深境螺旋每月两期」的规则已废弃。）标题是**合成**的，自带 MMDD：`深境螺旋0916` /
`幻想真境剧诗0901`——带日期是为了跨年时区分（主键 = (标题, 开始日期)）。
...
命中的条目跳过正文。实测 **20 次 → 2 次**（1 条对不上键的版本大活动 + 1 次维护预告）。
...
`ANNOUNCEMENT_ACTIVITY_TYPES`（含版本大活动）——改动前这里给候选预填「常规活动」凑主键，
版本大活动因此**每轮都被多抓一次**（实测 `「险境征者竞锋大赛」`）。
...
原因**（同一个接口被风控），不是平台封禁。实测**登录后浏览器可正常查看**。

> ⚠️ **但 cookie 不是确定的解**：本项目脚本在此前**未登录**的情况下成功抓取过大量正文，
> 之后才开始被拦。所以 1034 更像**行为/频率触发的风控**（连续多次运行是主因），登录态只是
> 提高信任等级、降低触发概率。遇到风控**首选等冷却**，cookie 是复发时的缓解手段，
> **真正治本的是 §4.3 的减少请求数**。
...
原神侧最需要记住的几点（**完整的三方对照表在
[../starrail/RULES.md](../starrail/RULES.md) §五**，这里只列影响原神自己的）：

| 项 | 原神 | 意味着什么 |
|---|---|---|
| **总纲** | **没有** | 版本更新 / 前瞻直播 / 深境螺旋 / 幻想真境剧诗靠**推理**（§3.1 / §3.2 / §3.8）；另两条线有总纲，能直接读 |
| 版本周期 | 固定 **42 天**、更新日固定**周三** | 可以外推；星铁周期不固定、绝区零公告明写 |
| 段落标记 | `〓活动时间〓` | 星铁 `▌`（三种写法）、绝区零 `【】` |
| 卡池标题 | 标题自带卡池名 + 5 星 | 另两条线要靠正文拼 |
| 大月卡 | 纪行（**每期名不同**） | 无名勋礼 / 丽都城募每期同名 → 标题必须带版本号 |
| 高难 | 幽境危战有公告；深境螺旋 / 幻想真境剧诗无公告 | 星铁的高难有权威公告（版本更新说明）；绝区零靠期数网格 |
| 风控对策 | 预筛（§4.3），**正文缓存未接** | 另两条线已接 `common/body_cache.py` |

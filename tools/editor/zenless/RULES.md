...
另两条管线的规则见 [../starrail/RULES.md](../starrail/RULES.md)（三条线差异见 §五）。

> **绝区零是三条线里信息结构最省事的一条**：每版本开服当天发的**停服更新公告**是一份
> 完整**总纲**——版本名、本版起止、下个版本的日期、本版全部活动的起止、高难期数，全在一篇里。
> 它是三条线里**唯一一个版本边界由公告明写、不用任何外推**的游戏：原神靠 42 天周期锚点推，
...
`python maintain.py`（跑三条管线），或单跑 `python zenless/pipeline.py`：
...
约定**；排除名单会过期，而写新公告的人不会通知我们。实测被后缀判据挡掉的：
`3.2版本「商城」上新说明`、`新剧情：主线第三季第三章「…」`、
...
- **`版本大活动` 与原神的判据无关**：这里只看「限时结束后收录进常驻」这个意思，**与时长无关**
  ...
  **不是每个版本都有版本大活动**——3.2 就没有，所以现有数据里 3.1 标「版本大活动」、
  3.2 标「常规活动」是**对的**。
...
> 网格实测：16 个手工日期里 **15 个吻合**。两处不符已定处置——`危局强袭战0729` 保留不改
> （见上），`式舆防卫战0807` 的结束日 08-21 与下期 08-21 重叠一天（按规则应为 08-20），
> 属既有录入误差，管线推出来的是 08-20。
...
「先遣赏金-区域巡防」早 **12 天**，「数据悬赏-实战模拟」早 **35 天**；而实测公告发布日
平均只比活动开始早 **2 天**。所以总纲优先让整版活动**在版本更新当天就挂上日历**。
...
| `2026/10/20 03:59` 结尾（惊喜放映企划） | 10-19 | **10-18** ✗（数据文件值也是错的） |
| `~ 3.1版本结束` = 09/09 06:00（法厄同年度大揭秘） | 09-08 | 09-09 ✗ |
| `2026/09/30 11:59`（3.2 卡池上期，白天时段） | **09-30** | 09-30 ✓ |

> **✗ 那 6 条不是「另一套口径」，是录入遗漏。** 同样是 `09/07 03:59` 结束，
> `极危通缉与悠游假期` 记成 09-06、`咔滋酥脆出餐计划` 记成 09-07——**自相矛盾**。
> 数据文件里 11 条「减了」、6 条「没减」，**管线按上面这条规则一律减一天**。
> 订正清单与处置见 [PLAN.md](PLAN.md) §6.2。
...
**去重主键**（`common/keys.event_key`，三条管线共用）：
...
**同名同类型**，老的 `(类型, 标题)` 认不出这是两期、新一期会被**静默跳过**（星铁「位面分裂」
当年就是这么丢的，后来靠一份名单补救；名单要人来维护，而活动名没有规律：3.1 两个福利槽叫
「云端礼赠」「『嗯呢』大派送！」，3.2 叫「全新放送」「『嗯呢』从天降」，四个名字互不相同、
也不复用）。所以这里**不建名单，直接给活动类带上日期**。
...
**没接正文缓存时**实测一轮：米游社列表 1 次 + 正文 12 次 + B站动态 2 页；预筛生效后稳态
正文请求是个位数。**2026-09-22 起接了本机缓存**（`common/body_cache.py`，TTL 14 天）：
`_fetch` 命中缓存就不发请求，稳态正文请求降到 **0**（实测第二轮：成功 0 / 缓存命中 5 /
跳过 22）。缓存里连 `content`（网页活动判据要用的原始外链）一起存。
设计说明与 TTL 取 14 天的理由见 [../starrail/PLAN.md](../starrail/PLAN.md) §1.5。
...
**完整的三方对照表在 [../starrail/RULES.md](../starrail/RULES.md) §五**，这里只列绝区零侧
最需要记住的：
